export function GooglePhotosIcon({ className = "" }: { className?: string }) {
  return (
    <svg
      width="59"
      height="59"
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 59 59"
      className={className}
    >
      <g fill="none" fillRule="evenodd">
        <path d="M-3-3h64v64H-3z" />
        <g fillRule="nonzero">
          <path
            d="M14.75 13.41c8.146 0 14.75 6.603 14.75 14.75v1.34H1.34C.6 29.5 0 28.9 0 28.16c0-8.147 6.604-14.75 14.75-14.75z"
            fill="#FBBC04"
          />
          <path
            d="M45.59 14.75c0 8.146-6.603 14.75-14.75 14.75H29.5V1.34C29.5.6 30.1 0 30.84 0c8.147 0 14.75 6.604 14.75 14.75z"
            fill="#EA4335"
          />
          <path
            d="M44.25 45.59c-8.146 0-14.75-6.603-14.75-14.75V29.5h28.16c.74 0 1.34.6 1.34 1.34 0 8.147-6.604 14.75-14.75 14.75z"
            fill="#4285F4"
          />
          <path
            d="M13.41 44.25c0-8.146 6.603-14.75 14.75-14.75h1.34v28.16c0 .74-.6 1.34-1.34 1.34-8.147 0-14.75-6.604-14.75-14.75z"
            fill="#34A853"
          />
        </g>
      </g>
    </svg>
  );
}

export function YouTubeIcon({ className = "" }: { className?: string }) {
  return (
    <svg
      width="159"
      height="110"
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 159 110"
      className={className}
    >
      <path
        d="M154 17.5c-1.82-6.73-7.07-12-13.8-13.8-9.04-3.49-96.6-5.2-122 .1-6.73 1.82-12 7.07-13.8 13.8C.32 35.5.01 74.2 4.5 92.5c1.82 6.73 7.07 12 13.8 13.8 17.9 4.12 103 4.7 122 0 6.73-1.82 12-7.07 13.8-13.8 4.35-19.5 4.66-55.8-.1-75z"
        fill="red"
      />
      <path d="M105 55 64.2 31.6v46.8z" fill="#fff" />
    </svg>
  );
}

export function FacebookSocialIcon({ className = "" }: { className?: string }) {
  return (
    <svg
      width="5"
      height="12"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      viewBox="0 0 5 12"
    >
      <path
        d="M3 5h1.5v1.5H3V10H1.5V6.5H0V5h1.5v-.628c0-.594.187-1.345.559-1.756A1.8 1.8 0 0 1 3.452 2H4.5v1.5H3.45a.45.45 0 0 0-.45.45V5Z"
        fill="currentColor"
      />
    </svg>
  );
}

export function YouTubeSocialIcon({ className = "" }: { className?: string }) {
  return (
    <svg
      stroke="currentColor"
      fill="currentColor"
      stroke-width="0"
      version="1.2"
      baseProfile="tiny"
      viewBox="0 0 24 24"
      height="1em"
      width="1em"
      className={className}
      xmlns="http://www.w3.org/2000/svg"
    >
      <path d="M22.8 8.6c-.2-1.5-.4-2.6-1-3-.6-.5-5.8-.6-9.8-.6s-9.2.1-9.8.6c-.6.4-.8 1.5-1 3s-.2 2.4-.2 3.4 0 1.9.2 3.4.4 2.6 1 3c.6.5 5.8.6 9.8.6 4 0 9.2-.1 9.8-.6.6-.4.8-1.5 1-3s.2-2.4.2-3.4 0-1.9-.2-3.4zm-12.8 7v-7.2l6 3.6-6 3.6z"></path>
    </svg>
  );
}
